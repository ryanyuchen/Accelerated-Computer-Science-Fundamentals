...
void LinkedList<T>::insertOrdered(const T& newData) {
  ...
  if (!head_) {
    // If empty, set the newNode to head and tail.
    head_ = newNode;
    tail_ = newNode;
  }
  else if (newData < head_->data) {
    // If newData < head, insert in front of head
    Node *oldHead = head_;
    ...
    // If newData > tail, insert after tail
    Node *oldTail = tail_;
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T>& other) const {
  ...
  LinkedList<T> &leftList = left.empty()? right : left;
  
  while (!leftList.empty()) {
    merged.pushBack(leftList.front());
    leftList.popFront();
  }
